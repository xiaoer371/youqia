...
- (NSString *)minuteDescription
{
    ...
    NSDateFormatter *dateFormatter1 = [NSDateFormatter dateFormatterWithFormat:@"yyyy-MM-dd"];
    
    NSInteger theYear = self.year;
    NSInteger currentYear = now.year;
    
    //NSString *theDayStr = [dateFormatter stringFromDate:self];//日期的年月日
	//NSString *currentDayStr = [dateFormatter stringFromDate:[NSDate date]];//当前年月日
    
    NSDate *theDay = [self dateWithOutTime:self];
    NSDate *currentDay = [self dateWithOutTime:now];
    
    if (theDay.year == currentDay.year &&
        theDay.month == currentDay.month &&
        theDay.day == currentDay.day) {//当天
		[dateFormatter setDateFormat:@"HH:mm"];
        return [dateFormatter stringFromDate:self];
        
	} else if ([currentDay timeIntervalSinceDate:theDay] == 86400) {//昨天
        ...
        return [NSString stringWithFormat:@"%@ %@",PMLocalizedStringWithKey(@"Yesterday"), [dateFormatter stringFromDate:self]];
        
    } else if ([currentDay timeIntervalSinceDate:theDay] < 86400 * 7) {//间隔一周内
        if (theYear < currentYear) {
            [dateFormatter setDateFormat:@"yyyy-MM-dd HH:mm"];
        }else {
...
//        return [dateFormatter stringFromDate:self];
        return [NSString stringWithFormat:@"%@ %@",[self getWeekdayWithDate:theDay],[dateFormatter1 stringFromDate:self]];
        
    } else if ([currentDay timeIntervalSinceDate:theDay] < 86400 * 365){//以前
        if (theYear < currentYear) {
            [dateFormatter setDateFormat:@"yyyy-MM-dd HH:mm"];
        }else {
...
-(NSString *)getWeekdayWithNumber:(NSInteger)number
{
    switch (number) {
        case 1:
            return PMLocalizedStringWithKey(@"Sunday");
            break;
        case 2:
            return PMLocalizedStringWithKey(@"Monday");
            break;
        case 3:
            return PMLocalizedStringWithKey(@"Tuesday");
            break;
        case 4:
            return PMLocalizedStringWithKey(@"Wednesday");
            break;
        case 5:
            return PMLocalizedStringWithKey(@"Thursday");
            break;
        case 6:
            return PMLocalizedStringWithKey(@"Friday");
            break;
        case 7:
            return PMLocalizedStringWithKey(@"Saturday");
            break;
        default:
            return @"";
            break;
