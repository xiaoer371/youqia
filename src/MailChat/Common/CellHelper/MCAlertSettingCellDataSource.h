//
//  MCAlertSettingCellDataSource.h
//  NPushMail
//
//  Created by wuwenyu on 16/4/22.
//  Copyright © 2016年 sprite. All rights reserved.
//

#import <Foundation/Foundation.h>

typedef enum : NSUInteger {
    MCNoticeAccountSetDefault,
    MCNoticeAccountSetDetail,
} MCNoticeAccountSetType;

typedef void (^ContactCellConfigureBlock)(id model, id cell, NSIndexPath *index);

@interface MCAlertSettingCellDataSource : NSObject<UITableViewDataSource>

- (id)initWithContactModels:(NSArray *)bmodels contactCellIdentifier:(NSString *)cellIdentifier configureCellBlock:(ContactCellConfigureBlock)aConfigureCellBlock;

- (id)modelAtIndexPath:(NSIndexPath *)indexPath;

@property (nonatomic, assign) MCNoticeAccountSetType type;

@end
