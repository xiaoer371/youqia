...
- (void)rightNavigationBarButtonItemAction:(id)sender {
    ...
    if ([_mcTextField.text trim].length == 0) {
        [SVProgressHUD showWithStatus:@"昵称不能为空"];
        return;
    }
   //TODO:保存昵称修改
    _account.displayName = _mcTextField.text;
    MCAccountManager *manager = [MCAccountManager new];
    [SVProgressHUD showWithStatus:@"正在保存..." maskType:SVProgressHUDMaskTypeClear];
    [manager updateAccount:_account withNickName:_mcTextField.text company:_account.company dept:_account.department title:_account.title success:^{
        DDLogDebug(@"修改昵称成功--");
