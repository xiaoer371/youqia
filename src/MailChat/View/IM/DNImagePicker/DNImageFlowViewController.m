...
- (void)browserPhotoAsstes:(NSArray *)assets pageIndex:(NSInteger)page
{
    ...
    browser.sendTitle = self.sendTitle;
...
- (BOOL)seletedAssets:(ALAsset *)asset
{
    ...
    if (self.selectedAssetsArray.count >= self.maxNumber ) {
...
- (UICollectionView *)imageFlowCollectionView
{
    if (nil == _imageFlowCollectionView) {
        ...
        _imageFlowCollectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height -NAVIGATIONBARHIGHT) collectionViewLayout:layout];
...
- (DNSendButton *)sendButton
{
    if (nil == _sendButton) {
        ...
        _sendButton.title = self.sendTitle;
