...
@property (nonatomic, strong) UIImageView *bombImageView;
...
{
    RTDraggableBadge *badge = [[RTDraggableBadge alloc] initWithFrame:CGRectMake(0, 0, 19, 19)];
...
- (void)commonInit
{
    ...
    self.font = [RTDraggableBadge appearance].font ?: [UIFont systemFontOfSize:11.f];
...
{
    if ([text isEqualToString:@"0"]||text.length<1) {
        self.hidden = YES;
    }else{
        self.hidden = NO;
    }
...
- (UIImageView *)bombImageView
{
    if (!_bombImageView) {
        _bombImageView = [[UIImageView alloc] initWithFrame:self.bounds];
        _bombImageView.backgroundColor = [UIColor clearColor];
        _bombImageView.animationImages = @[[UIImage imageNamed:@"RTDraggableBadge.bundle/bomb0"],
                                           [UIImage imageNamed:@"RTDraggableBadge.bundle/bomb1"],
                                           [UIImage imageNamed:@"RTDraggableBadge.bundle/bomb2"],
                                           [UIImage imageNamed:@"RTDraggableBadge.bundle/bomb3"],
                                           [UIImage imageNamed:@"RTDraggableBadge.bundle/bomb4"]];
        _bombImageView.animationRepeatCount = 1;
        _bombImageView.animationDuration = 1;
    }
    return _bombImageView;
}
/*
...
{
    return;
    UIWindow *mailWindow = [[UIApplication sharedApplication].delegate window];
    [mailWindow addSubview:self.bombImageView];
    CGPoint point = [tap locationInView:mailWindow];
    self.bombImageView.center = point;
    [_bombImageView startAnimating];
    
    if (self.doubleClickblock) {
        self.doubleClickblock(nil);
    }
    self.text = @"";
    [self performSelector:@selector(resetText:) withObject:nil afterDelay:1];
}
...
- (void)startbomAnimation
{
    UIWindow *mailWindow = [[UIApplication sharedApplication].delegate window];
    [mailWindow addSubview:self.bombImageView];
    CGRect rect=[self convertRect: self.bounds toView:mailWindow];
    CGFloat x =rect.origin.x + rect.size.width/2;
    CGFloat y =rect.origin.y + rect.size.height/2;
    self.bombImageView.center = CGPointMake(x,y);
    [_bombImageView startAnimating];
    self.text = @"";
}
...
- (void)onPan:(UIPanGestureRecognizer *)pan
{
    switch (pan.state) {
        case UIGestureRecognizerStateBegan:
        {
            ...
            if (![self.containerView.subviews containsObject:self]) {
        ...
        {
            if ([self isBomDistance]) {
                
                UIWindow *mailWindow = [[UIApplication sharedApplication].delegate window];
                [mailWindow addSubview:self.bombImageView];
                CGPoint point = [pan locationInView:mailWindow];
                self.bombImageView.center = point;
                [_bombImageView startAnimating];
                if (self.delegate &&[self.delegate respondsToSelector:@selector(dragOut:)]) {
                    [self.delegate dragOut:self];
                }
                
                if (self.dragOutblock) {
                    self.dragOutblock(nil);
                }
                self.text = @"";
                      ...
                      initialSpringVelocity:10.f
                                 ...
                                 completion:^(BOOL finished) {
                                     if (finished) {
                                         ...
                                         if (![self.originSuperView.subviews containsObject:self]) {
...
- (void)resetText:(id)sender
{
    ...
    [self.bombImageView removeFromSuperview];
    self.bombImageView = nil;
}
