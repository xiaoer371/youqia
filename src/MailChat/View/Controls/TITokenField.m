...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if ((self = [super initWithFrame:frame])){
        ...
        _lineVi = [[UIView alloc]initWithFrame:CGRectMake(10, frame.size.height - 0.5, ScreenWidth -10 , 0.5)];
        _lineVi.backgroundColor = [UIColor colorWithHexString:@"dedfe0"];
        [self addSubview:_lineVi];
        self.rightView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, 30, frame.size.height)];
...
- (void)setup {
    [self setTextColor:[UIColor colorWithHexString:@"777777"]];
	[self setBorderStyle:UITextBorderStyleNone];
	[self setFont:[UIFont systemFontOfSize:17]];
	[self setBackgroundColor:AppStatus.theme.backgroundColor];
...
- (void)didEndEditing {
	...
	if (_removesTokensOnEndEditing){
		...
		if (_tokens.count){
			...
			if (_tokens.count > 1 && untokSize.width > availableWidth - 60){
				untokenized = [NSString stringWithFormat:@"%lu个联系人", (unsigned long)titles.count];
			}
...
- (TIToken *)addTokenWithTitle:(NSString *)title representedObject:(id)object {
	...
	if (title.length){
		TIToken * token = [[TIToken alloc] initWithTitle:title representedObject:object font:nil];
		[self addToken:token];
		return token;
	}
...
- (void)addTokensWithTitleArray:(NSArray *)titleArray {
    for (id obj in titleArray) {
        if ([obj isKindOfClass:[MCMailAddress class]]) {
            MCMailAddress *mailAddress = (MCMailAddress*)obj;
            [self addTokenWithTitle:mailAddress.name representedObject:mailAddress];
        } else {
            [self addTokenWithTitle:obj];
        }
    }
}
...
- (void)tokenTouchUpInside:(TIToken *)token {
    ...
    if (token.selected) {
       NSLog(@"tokem");
        if ([delegate respondsToSelector:@selector(tokenField:didTouchUpInsideToken:)]) {
            [delegate tokenField:self didTouchUpInsideToken:token];
        }
    }
...
- (CGFloat)layoutTokensInternal {
	...
	[_tokens enumerateObjectsUsingBlock:^(TIToken * token, NSUInteger idx, BOOL *stop){
		...
		[token setMaxWidth:(self.bounds.size.width - rightMargin -100- (_numberOfLines > 1 ? hPadding : leftMargin))];
		...
		if (token.superview){
			...
			[token setFrame:(CGRect){_tokenCaret.x+3,_tokenCaret.y, token.bounds.size}];
			_tokenCaret.x += token.bounds.size.width + 4;
			
			if (self.bounds.size.width - _tokenCaret.x - rightMargin < 30){
				_numberOfLines++;
				_tokenCaret.x = (_numberOfLines > 1 ? hPadding : leftMargin);
				_tokenCaret.y += lineHeight;
			}
...
- (void)layoutTokensAnimated:(BOOL)animated {
	...
	if (self.bounds.size.height != newHeight){
		...
		[UIView animateWithDuration:(animated ? 0.3 : 0) animations:^{
			...
			_lineVi.frame = CGRectMake(10, newHeight - 2, ScreenWidth - 10, 0.5);
...
- (CGRect)textRectForBounds:(CGRect)bounds {
	...
	CGRect frame = CGRectOffset(bounds, _tokenCaret.x + 4, _tokenCaret.y + 2);
...
- (CGRect)leftViewRectForBounds:(CGRect)bounds {
	return ((CGRect){10.0f, 0, self.leftView.bounds.size.width,45.0});
}
...
static NSString*const kTITokenHightLightColorValue = @"4284d9";
...
+ (UIColor *)blueTintColor {
    //TODO:联系人颜色
    return [UIColor colorWithHexString:@"76b2ff"];
}
...
- (void)drawRect:(CGRect)rect {
	...
	CGFloat blue = 1;
	CGFloat alpha = 1;
	[self getTintColorRed:&red green:&green blue:&blue alpha:&alpha color:_tintColor];
    ...
    CGFloat blue2 = 1;
    CGFloat alpha2 = 1;
    [self getTintColorRed:&red2 green:&green2 blue:&blue2 alpha:&alpha2 color:[UIColor colorWithHexString:kTITokenHightLightColorValue]];
    CGFloat highlightedComp[8] = {red2, green2, blue2, 0.9, red, green, blue, 1};
    CGFloat nonHighlightedComp[8] = {red, green, blue, 0.9, red, green, blue, 1};
