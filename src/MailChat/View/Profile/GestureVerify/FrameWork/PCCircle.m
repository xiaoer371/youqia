 ...
 *  外实心圆颜色
 */
@property (nonatomic, strong) UIColor *outSolidCircleColor;
...
- (void)drawRect:(CGRect)rect {
    ...
    //TODO 增加外实心圆绘制
    [self drawOutSolidCircleWithContext:ctx rect:circleRect color:self.outSolidCircleColor];
 ...
 *  @param color
 */
- (void)drawOutSolidCircleWithContext:(CGContextRef)ctx rect:(CGRect)rect color:(UIColor *)color {
    CGMutablePathRef circlePath = CGPathCreateMutable();
    CGPathAddEllipseInRect(circlePath, NULL, rect);
    CGContextAddPath(ctx, circlePath);
    [color set];
...
- (UIColor *)outSolidCircleColor {
    UIColor *color;
    switch (self.state) {
        case CircleStateNormal:
            color = CircleStateNormalOutsolidColor;
            break;
        case CircleStateSelected:
            color = CircleStateSelectedOutsolidColor;
            break;
        case CircleStateError:
            color = CircleStateNormalOutsolidColor;
            break;
