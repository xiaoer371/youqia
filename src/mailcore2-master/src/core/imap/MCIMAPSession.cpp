...
IndexSet * IMAPSession::search(String * folder, IMAPSearchKind kind, String * searchString, ErrorCode * pError)
{
    ...
    switch (kind) {
        ...
        {
            expr = IMAPSearchExpression::searchFrom(searchString,1);
            break;
        }
        ...
        {
            expr = IMAPSearchExpression::searchSubject(searchString,1);
            break;
        }
...
IndexSet * IMAPSession::search(String * folder, IMAPSearchExpression * expression, ErrorCode * pError)
{
    ...
    //TODO:判断35哪个域的账号
    if (expression->searchFor35Type()==2.0) {
        charset = "gb2312";
...
{
    String * vendor = MCSTR("tencent limited");
    String * name = MCSTR("com.tencent.foxmail");
    String * version = MCSTR("7.2.6.42");
    String * info1 = MCSTR( "windows");
    String * key1 = MCSTR("os");
    String * info2 = MCSTR("6.1");
    String * key2 = MCSTR("os-version");
    String * info3 = MCSTR("foxmail@foxmail.com");
    String * key3 = MCSTR("contact");
    
    mClientIdentity ->setVendor(vendor);
    mClientIdentity ->setName(name);
    mClientIdentity ->setVersion(version);
    mClientIdentity ->setInfoForKey(key1,info1);
    mServerIdentity ->setInfoForKey(key2,info2);
    mClientIdentity ->setInfoForKey(key3,info3);
