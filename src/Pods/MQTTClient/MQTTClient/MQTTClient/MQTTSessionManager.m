...
- (void)addSubscriptions:(NSDictionary<NSString *,NSNumber *> *)addedSubscription complete:(MQTTSubscribeHandler)completion
{
    NSMutableDictionary *newAppendedSubscriptions = [NSMutableDictionary new];
    if (self.state == MQTTSessionManagerStateConnected) {
        for (NSString *topic in addedSubscription) {
            if (![self.effectiveSubscriptions objectForKey:topic]) {
                NSNumber *gQos = addedSubscription[topic];
                [newAppendedSubscriptions setObject:gQos forKey:topic];
            }
        }
        
        __weak typeof(self) weakSelf = self;
        [self.session subscribeToTopics:newAppendedSubscriptions subscribeHandler:^(NSError *error, NSArray<NSNumber *> *gQoss) {
            if (!error) {
                @synchronized (weakSelf) {
                    NSMutableDictionary *newEffectiveSubscriptions = [weakSelf.effectiveSubscriptions mutableCopy];
                    [newEffectiveSubscriptions addEntriesFromDictionary:newAppendedSubscriptions];
                    weakSelf.effectiveSubscriptions = [newEffectiveSubscriptions copy];
                }
            }
            
            if (completion) {
                completion(error,gQoss);
            }
    ...
    @synchronized (self) {
        ...
        [newInternalSubscriptions addEntriesFromDictionary:newAppendedSubscriptions];
        self.internalSubscriptions = [newInternalSubscriptions copy];
        DDLogVerbose(@"MQTTSessionManager internalSubscriptions: %@", self.internalSubscriptions);
    }
}
