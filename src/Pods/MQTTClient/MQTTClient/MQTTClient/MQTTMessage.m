                ...
                if ((type == MQTTPublish && (qos >= MQTTQosLevelAtMostOnce && qos <= MQTTQosLevelExactlyOnce)) ||
                    ...
                    if (type == MQTTPuback ||
                        ...
                        if (message.data.length > 2) {
                            ...
                            //GAOYQ: Since the server returns server time on pubrec, the data lenth is more than 2 bytes.
                            //message = nil;
                            //TODO: Parse server data here.
                            message.data = [message.data subdataWithRange:NSMakeRange(0, 2)];
                        }
                    }
                    if (type == MQTTPingreq ||
                        type == MQTTPingresp ||
