...
@interface OIDAuthorizationUICoordinatorIOS ()<SFSafariViewControllerDelegate,UIWebViewDelegate>
@end
...
@implementation OIDAuthorizationUICoordinatorIOS {
  ...
  __weak OIDWebViewController *_webViewVC;
}
...
- (BOOL)presentAuthorizationWithURL:(NSURL *)URL session:(id<OIDAuthorizationFlowSession>)session {
  ...
  _session = session;
    ...
    OIDWebViewController *webViewController = [[OIDWebViewController alloc] initWithUrl:URL];
    _webViewVC = webViewController;
    [_presentingViewController presentViewController:webViewController animated:YES completion:nil];
    return YES;
...
- (void)dismissAuthorizationAnimated:(BOOL)animated completion:(void (^)(void))completion {
  ...
  OIDWebViewController *safariVC = _webViewVC;
  [self cleanUp];
  if (safariVC) {
